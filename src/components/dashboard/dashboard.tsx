...
export function Dashboard({ user, onNavigate, onboardingStepJustDone, onOnboardingStepDoneConsumed }: DashboardProps) {
  ...
  const [vatRegister, setVatRegister] = useState<{ totalOutputVAT?: number; totalInputVAT?: number; netVAT?: number } | null>(null);
  ...
  const invoiceStats = useMemo(() => {
    ...
    const outstandingTotal = outstandingInvoices.reduce((sum, inv) => sum + (inv.total || 0), 0);
  ...
  const vatBreakdown = useMemo(() => {
    // When VAT register data is available (double-entry bookkeeping), use it
    // for accurate per-rate breakdown instead of estimating from transactions
    if (vatRegister?.totalOutputVAT !== undefined) {
      // The VAT register API returns the actual per-code VAT amounts;
      // derive a rough rate-to-amount mapping for the pie chart.
      // Use the transactions for count/context but the register for VAT values.
      const registerTotal = vatRegister.totalOutputVAT || 0;
      // Count sales by VAT rate from transactions
      const rateCounts: Record<number, number> = {};
      salesThisMonth.forEach((t) => {
        rateCounts[t.vatPercent] = (rateCounts[t.vatPercent] || 0) + 1;
      });
      // If we have rate-count info, distribute proportionally
      const totalTxCount = salesThisMonth.length || 1;
      const entries = Object.entries(rateCounts);
      if (entries.length > 0) {
        return entries.map(([rate, count]) => {
          const fraction = count / totalTxCount;
          return {
            name: `${rate}%`,
            rate: parseFloat(rate),
            amount: 0,
            vat: Math.round(registerTotal * fraction * 100) / 100,
            count,
          };
        });
      }
      return [{ name: '25%', rate: 25, amount: 0, vat: registerTotal, count: salesThisMonth.length }];
    }

    // No VAT register data and no transactions — return empty breakdown
    return [];
  }, [salesThisMonth, vatRegister]);
  ...
  const WidgetPicker = () => (
    <Dialog open={widgetPickerOpen} onOpenChange={setWidgetPickerOpen}>
      ...
      <DialogContent className="max-w-md max-h-[85vh] overflow-y-auto bg-white dark:bg-[#1a1f1e]">
        <DialogHeader>
  ...
  return (
    <div className="p-3 sm:p-4 lg:p-8 space-y-4 lg:space-y-6">
      {/* ═══════════════════════════════════════════════════════════
            ...
            {onboardingSteps.map((step) => {
              ...
              return (
                ...
                >
                  ...
                  <CardContent className="relative z-10 p-5 pt-[65px] h-full flex flex-col">
