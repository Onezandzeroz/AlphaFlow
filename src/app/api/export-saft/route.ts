...
const OUTPUT_VAT_CODES = ['S25', 'S12', 'S0', 'SEU'];
const INPUT_VAT_CODES = ['K25', 'K12', 'K0', 'KEU', 'KUF'];
...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const code of allVatCodes) {
      const data = vatCodeMap.get(code);
      if (data && (data.debitTotal > 0 || data.creditTotal > 0)) {
        const isOutput = OUTPUT_VAT_CODES.includes(code);
      ...
      salesRelatedEntries.forEach((entry, index) => {
        ...
        salesLines.forEach((line, lineIdx) => {
          ...
          if (vatCode !== 'NONE') {
            ...
            invTax
              .ele('TaxPercentage')
              .txt((VAT_RATES[vatCode] ?? 0).toString());
