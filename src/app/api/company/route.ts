...
export async function POST(request: NextRequest) {
  try {
    ...
    await auditCreate(ctx.id, 'Company', company.id, { companyName, cvrNumber }, requestMetadata(request));
...
export async function PUT(request: NextRequest) {
  try {
    ...
    await auditUpdate(ctx.id, 'Company', existing.id, oldData, newData, requestMetadata(request));
