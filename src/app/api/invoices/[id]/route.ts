...
async function createAccrualJournalEntry(
  ...
  for (const item of lineItems) {
    ...
    // Credit the revenue account specified on the line item
    if (item.accountId) {
      const vatCode = item.vatPercent === 25 ? 'S25' : item.vatPercent === 12 ? 'S12' : undefined;
      jeLines.push({
      ...
      logger.warn(`[Invoice SENT] Line item "${item.description}" has no accountId. Revenue credit will be missing for ${existing.invoiceNumber}.`);
    }
...
export async function PUT(
  ...
  try {
    ...
    // Audit log
    await auditUpdate(ctx.id, 'Invoice', id, oldData, newData, requestMetadata(request));
